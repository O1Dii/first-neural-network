...
prices = np.array([i ** 2 / 1000000 for i in squareFeet], dtype="int32")

model = keras.Sequential()
model.add(keras.layers.Dense(20, input_dim=1, activation='linear'))
model.add(keras.layers.Dense(200, input_dim=1, activation='elu'))
model.add(keras.layers.Dense(200, input_dim=1, activation='elu'))
